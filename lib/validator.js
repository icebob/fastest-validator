...
class Validator {
	...
	wrapRequiredCheckSourceCode(rule, innerSrc, context, resVar) {
		...
		if (rule.schema.default != null) {
			// We should set default-value when value is undefined or null, not skip!
			skipUndefinedValue = false;
			skipNullValue = false;
			let defaultValue = JSON.stringify(rule.schema.default);
			...
			handleNoValueSrc = this.makeError({ type: "required", actual: "value", messages: rule.messages });
		}
