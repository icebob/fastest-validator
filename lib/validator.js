...
Validator.prototype.compile = function(schema, parentPath) {
	...
	const processRule = function processRule(rule, name, path, iterate = false) {
		...
		if (rule.type === "object" && rule.props) {
			...
			checks.push({
				fn: self.compile(rule.props, path),
				type: rule.type,
				name: name,
				schema: rule,
				path: path,
		...
		if (rule.type === "array" && rule.items) {
			// Compile the child schema
			processRule(rule.items, name, path, true);
		}		
	};
	...
	Object.keys(schema).forEach(name => {
		...
		processRule(schema[name], name, path);
	});
	...
	return function(obj, _schema, pathStack = "") {
		...
		for (let i = 0; i < count; i++) {
			...
			//const path = check.path; // + (idx !== undefined ? `[${idx}]` : "");
			//let stack = check.path; //pathStack.concat(check.name);
			//const stack = []; // pathStack.concat(check.name);
			/*for(let i = 0; i < pathStack.length; i++)
				stack.push(pathStack[i]);
			stack.push(check.name);
			*/
