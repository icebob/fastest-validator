...
Validator.prototype.compile = function(schema) {
	...
	const sourceCode = [
		"const errors = [];",
		"let value = data;", // TODO: path handling
	];

	const rule = this.compileSchemaRule(schema);
	sourceCode.push(rule.ruleFunction.call(this, rule.schema, null, rule.messages));
	...
	const checkFn = new Function("data", src);
...
/*
Validator.prototype.generateSchemaObjectChecker = function(schemaObject, parentPath) {
	...
	for (let i = 0; i < properties.length; i++) {
		...
		const path = (parentPath ? parentPath + "." : "") + property;
		...
		if (rule.schema.optional === true || rule.schema.type == "forbidden") {
			...
			sourceCode.push(`
			if (value === undefined || value === null) {
				errors.push({ type: "required", field: "${path}" });
			} else {`);
		}

		sourceCode.push(rule.ruleFunction.call(this, rule.schema, path));
...
Validator.prototype.escapeEvalString = function(str) {
...
Validator.prototype.identifierRegex = identifierRegex;
