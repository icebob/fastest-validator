...
const validateRuleProperties = require("./helpers/validateRuleProperties");
...
class Validator {
	...
	compileRule(rule, context, path, innerSrc, resVar) {
		...
		if (item) {
			...
			if (this.strict && res.propsSchema) {
				const result = validateRuleProperties(Validator, res.propsSchema, rule.schema);
				if (Array.isArray(result)) this.showWarnOrThrow(`Error on compiling rule: ${result[0].message}`);
			}
	...
	showWarnOrThrow(message) {
		if (this.strict === "warn") return this.logger.warn(message);
