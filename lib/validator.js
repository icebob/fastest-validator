...
Validator.prototype.compile = function(schema) {
	...
	if (res.source) {
		...
		sourceCode.push(this.wrapRequiredCheckSourceCode(rule, `
			context.fn[${context.index}](value, null, null, errors, context);
		`));
	...
	if (this.opts.debug) {
		//console.log(src);
		const prettier = require("./helpers/prettier");

		context.fn.forEach((fn, i) => {
			console.log(`// Context.fn[${i}]`);
			console.log(prettier(fn.toString()));
		});

		console.log("// Main check function");
		console.log(prettier(checkFn.toString()));
	}
