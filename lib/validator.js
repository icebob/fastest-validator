...
Validator.prototype.compileSchemaObject = function(schemaObject, nullable) {
	...
	if (compiledObject) {
		...
		compiledObject = { cycle: false, nullable, properties: null, compiledObjectFunction: null, objectStack: [] };
	...
	compiledObject.properties = Object.keys(schemaObject)
		...
		.map(name => {
			const compiledType = this.compileSchemaType(schemaObject[name], nullable);
...
Validator.prototype.compileSchemaType = function(schemaType, nullable) {
	...
	return this.compileSchemaRule(schemaType, nullable);
...
Validator.prototype.compileSchemaRule = function(schemaRule, nullable) {
	...
	if (schemaRule.type === "object" && schemaRule.props) {
		let nullable = false;
		if (schemaRule.optional || schemaRule.nullable) {
			nullable = true;
			schemaRule.nullable = nullable;
		}
		dataParameter = this.compileSchemaObject(schemaRule.props, nullable);
...
Validator.prototype.checkSchemaRule = function(value, compiledRule, path, parent) {
	...
	if (value === undefined || value === null) {
		...
		if (schemaRule.optional === true || schemaRule.nullable === true)
			return true;
