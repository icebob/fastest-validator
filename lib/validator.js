...
} catch(err) { /* eslint-disable-line no-unused-vars -- async is not supported */}

const deepExtend = require("./helpers/deep-extend");
const replace = require("./helpers/replace");
const { safeStringLiteral, hasOwn } = require("./helpers/schema-options");

/**
 * Maximum allowed nesting depth for schemas (compile & normalize). Prevents
 * stack overflows (RangeError) from deeply nested attacker-supplied schemas.
 */
...
class Validator {
	...
	compile(schema) {
		...
		const sourceCode = [
			...
			"var field;",
			"var parent = null;",
			`var label = ${schema.label != null ? safeStringLiteral(schema.label) : "null"};`
		];
	...
	compileRule(rule, context, path, innerSrc, resVar) {
		...
		if (item) {
			...
			if (context.depth > MAX_SCHEMA_DEPTH) {
				throw new Error("Invalid schema: maximum nesting depth of 100 exceeded");
			}
			context.depth++;
	...
	getRuleFromSchema(schema) {
		...
		const alias = hasOwn(this.aliases, schema.type) ? this.aliases[schema.type] : undefined;
		if (alias) {
			delete schema.type;
			schema = deepExtend(schema, alias, { skipIfExist: true });
		}

		const ruleFunction = hasOwn(this.rules, schema.type) ? this.rules[schema.type] : undefined;
		if (!ruleFunction)
			throw new Error("Invalid '" + schema.type + "' type in validator schema.");

		const rule = {
			messages: Object.assign({}, this.messages, schema.messages),
			schema: deepExtend(schema, hasOwn(this.defaults, schema.type) ? this.defaults[schema.type] : {}, { skipIfExist: true }),
	...
	makeError({ type, field, expected, actual, messages }) {
		const o = {
			...
			message: JSON.stringify(messages[type]),
		};
		if (field) o.field = JSON.stringify(field);
		else o.field = "field";
			...
			if (this.opts.useNewCustomCheckerFunction) {
				...
				if(Array.isArray(schema[fnName])){
					for (let i = 0; i < schema[fnName].length; i++) {
						...
						if(custom.type){
							if (!hasOwn(context.customFunctions, custom.type)) {
								throw new Error("Invalid '" + custom.type + "' type in custom validator.");
							}
							ret.push( `
						 ${vName} = ${context.async ? "await " : ""}context.customFunctions[${ruleVName}.schema.${fnName}[${i}].type].call(this, ${vName}, ${fnCustomErrorsVName} , ${ruleVName}_${i}.schema, ${safeStringLiteral(path)}, parent, context);
						`);
						}
						if(typeof custom==="function"){
							ret.push( `
						${vName} = ${context.async ? "await " : ""}${ruleVName}.schema.${fnName}[${i}].call(this, ${vName}, ${fnCustomErrorsVName} , ${ruleVName}.schema, ${safeStringLiteral(path)}, parent, context);
						`);
						}
					}
				}else{
					ret.push( `
				${vName} = ${context.async ? "await " : ""}${ruleVName}.schema.${fnName}.call(this, ${vName}, ${fnCustomErrorsVName} , ${ruleVName}.schema, ${safeStringLiteral(path)}, parent, context);
				`);
				}
				...
				ret.push( `
				const ${ruleVName} = context.customs[${ruleIndex}];
				const ${result} = ${context.async ? "await " : ""}${ruleVName}.schema.${fnName}.call(this, ${vName}, ${ruleVName}.schema, ${safeStringLiteral(path)}, parent, context);
	...
	alias(name, validationRule) {
		if (hasOwn(this.rules, name)) throw new Error("Alias name must not be a rule name");
	...
	normalize(value, depth = 0) {
		...
		if (hasOwn(this.aliases, result.type)) {
			const aliasValue = this.aliases[result.type];
			delete result.type;
			result = deepExtend(result, this.normalize(aliasValue, depth + 1), { skipIfExist: true });
		}

		if (hasOwn(this.defaults, result.type)) {
			result = deepExtend(result, this.defaults[result.type], { skipIfExist: true });
		}

		if (result.type === "multi") {
			result.rules = result.rules.map(r => this.normalize(r, depth + 1));
			result.optional = result.rules.every(r => r.optional === true);
			return result;
		}
		if (result.type === "array") {
			result.items = this.normalize(result.items, depth + 1);
			return result;
		}
		if (result.type === "object") {
			if (result.props) {
				Object.entries(result.props).forEach(([k,v]) => result.props[k] = this.normalize(v, depth + 1));
			}
		}
		if (typeof value === "object") {
			if (value.type) {
				// A scalar type shorthand is resolved inline (same level); an
				// object-typed 'type' descends structurally and counts a level.
				const config = typeof value.type === "string" ?
					this.normalize(value.type, depth) :
					this.normalize(value.type, depth + 1);
				deepExtend(result, config, { skipIfExist: true });
			}
			else{
				Object.entries(value).forEach(([k,v]) => result[k] = this.normalize(v, depth + 1));
			}
		}
