...
Validator.prototype.compile = function(schema) {
	...
	// Multiple schemas
		...
		return function(value, path, parent) {
			const result = self.checkSchemaType(value, rules, path, parent || null);
			return result.success ? true : result.errors;
		};
	}
	...
	this.cache.clear();
	return function(value, path, parent) {
		const result = self.checkSchemaObject(value, rule, path, parent || null);
		return result.success ? true : result.errors;
	};
};
...
Validator.prototype.compileSchemaObject = function(schemaObject) {
	...
	for (let i = 0; i < compiledObject.properties.length; i++) {
		...
		sourceCode.push("if (res.success !== true) {");
		sourceCode.push(`\tthis.handleResult(errors, propertyPath, res.errors, properties[${i}].compiledType.messages);`);
		sourceCode.push("}");
		sourceCode.push("else {");
		sourceCode.push(`\t${propertyValueExpr} = res.convertValue !== undefined ? res.convertValue : ${propertyValueExpr};`);
		sourceCode.push("}");
	...
	sourceCode.push("return errors.length === 0 ? {success: true} : {success: false, errors: errors};");
...
Validator.prototype.checkSchemaType = function(value, compiledType, path, parent) {
	...
	if (Array.isArray(compiledType)) {
		...
		for (let i = 0; i < checksLength; i++) {
			...
			if (res.success !== true) {
				this.handleResult(errors, path, res.errors, compiledType.messages);
			} else {
...
Validator.prototype.checkSchemaArray = function(value, compiledArray, path, parent) {
	...
	for (let i = 0; i < valueLength; i++) {
		...
		if (res.success !== true) {
			this.handleResult(errors, itemPath, res.errors, compiledArray.messages);
		}
	}

	return errors.length === 0 ? {success: true} : {success: false, errors: errors};
};
...
Validator.prototype.checkSchemaRule = function(value, compiledRule, path, parent) {
	...
	if (schemaRule.convert === true) {
		if (schemaRule.type === "boolean") return {success: true, convertValue: convert.boolean(value)};
		if (schemaRule.type === "number") return {success: true, convertValue: convert.number(value)};
	}
	return {success: true};
};
