...
Validator.prototype.checkSchemaType = function(value, compiledType, path, parent) {
	...
	if (Array.isArray(compiledType)) {
		...
		for (let i = 0; i < checksLength; i++) {
			...
			if (res !== true) {
				this.handleResult(errors, path, res, compiledType.messages, null);
			} else {
...
Validator.prototype.checkSchemaArray = function(value, compiledArray, path, parent) {
	...
	for (let i = 0; i < valueLength; i++) {
		...
		if (res !== true) {
			this.handleResult(errors, itemPath, res, compiledArray.messages, null);
		}
	}
...
Validator.prototype.checkSchemaRule = function(value, compiledRule, path, parent) {
	...
	if (value === undefined || value === null) {
		...
		const errors = [];
		this.handleResult(errors, path, this.makeError("required"), compiledRule.messages, schemaRule.displayName);
	...
	if (res !== true) {
		const errors = [];
		this.handleResult(errors, path, res, compiledRule.messages, schemaRule.displayName);
 ...
 */
Validator.prototype.handleResult = function(errors, fieldPath, res, messages, fieldDisplayName) {
	let items;
	...
	items.forEach(err => {
		if (!err.displayName && fieldDisplayName)
			err.displayName = fieldDisplayName;
		if (!err.field)
			err.field = fieldPath;
		if (!err.message)
...
Validator.prototype.resolveMessage = function(err, msg = null) {
	...
	if (msg != null) {
		...
		return msg.replace(/\{field\}/g, err.displayName || err.field).replace(/\{expected\}/g, expected).replace(/\{actual\}/g, actual);
	}
};
