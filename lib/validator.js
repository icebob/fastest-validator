...
Validator.prototype.checkSchemaRule = function(value, compiledRule, path, parent) {
	...
	if (value === undefined || value === null) {
		...
		const errors = [];
		this.handleResult(errors, path, this.makeError("required"), compiledRule.messages, schemaRule.display);
	...
	if (res !== true) {
		const errors = [];
		this.handleResult(errors, path, res, compiledRule.messages, schemaRule.display);
 ...
 */
Validator.prototype.handleResult = function(errors, fieldPath, res, messages, fieldDisplay) {
	let items;
	...
	items.forEach(err => {
		if (!err.display && fieldDisplay)
			err.display = fieldDisplay;
		if (!err.field)
			err.field = fieldPath;
		if (!err.message)
...
Validator.prototype.resolveMessage = function(err, msg = null) {
	...
	if (msg != null) {
		...
		return msg.replace(/\{field\}/g, err.display || err.field).replace(/\{expected\}/g, expected).replace(/\{actual\}/g, actual);
	}
};
