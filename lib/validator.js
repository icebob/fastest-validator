...
class Validator {
	 ...
	 */
	validateAsync(obj, schema) {
		const check = this.compileAsync(schema);
		return check(obj);
	}
	 ...
	 */
	compileAsync(schema) {
		return this.compile(schema, { async: true });
	}
	...
	compile(schema, opts = {}) {
		...
		const context = {
			index: 0,
			async: !!opts.async || !!this.opts.async,
			rules: [],
			fn: [],
			customs: {}
		};
