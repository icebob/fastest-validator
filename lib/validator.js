...
Validator.prototype.compile = function(schema) {
	...
	return self.compileSchemaObject2(schema);
};

Validator.prototype.compileSchemaObject2 = function(schemaObject) {
	...
	// TODO: caching for cycling
	let path;
	const properties = Object.keys(schemaObject);

	const sourceCode = [
		"let res;",
		"const errors = [];",
		"let obj = data;", // TODO: path handling
		//"let field, value;",
	];
	...
	for (let i = 0; i < properties.length; i++) {
		...
		sourceCode.push("if (true) {");

		const rule = this.compileSchemaRule(schemaObject[property]);
		const name = escapeEvalString(property);
		const safePropName = identifierRegex.test(name) ? `obj.${name}` : `obj["${name}"]`;
		//const valueName = `value_${cnt}`;

		sourceCode.push(`\n\t// Property: ${property}`);
		sourceCode.push(`\tlet field = "${name}";`);
		sourceCode.push(`\tlet value = ${safePropName};`);
		...
		if (rule.schema.optional === true || rule.schema.type == "forbidden") {
			...
			sourceCode.push(`
			if (value === undefined || value === null) {
				errors.push({ type: "required", field });
			} else {`);
		}

		sourceCode.push(rule.ruleFunction(rule.schema));

		//if (rule.schema.optional !== true)

		sourceCode.push("\t}"); // Required, optional
		sourceCode.push("}"); // if (true)
	...
	sourceCode.push("\t return this.humanizeErrors(errors);");
	sourceCode.push("}");
	sourceCode.push("return true;");

	console.log(sourceCode.join("\n"));

	const checkFn = new Function("data", sourceCode.join("\n"));
	return function(data) {
		return checkFn.call(self, data);
	};
};
/*
...
Validator.prototype.compileSchemaRule = function(schema) {
	...
	const messages = Object.assign({}, this.messages, schema.messages);

	/*let dataParameter = null;
	let dataFunction = null;

	if (schema.type === "object" && schema.props) {
		dataParameter = this.compileSchemaObject(schema.props);
		dataFunction = this.checkSchemaObject;
	} else if (schema.type === "array" && schema.items) {
		dataParameter = this.compileSchemaType(schema.items);
