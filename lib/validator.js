...
class Validator {
	...
	constructor(opts) {
		...
		if (opts) {
			...
			if (!this.opts.useNewCustomCheckerFunction) console.warn("DeprecationWarning: set 'useNewCustomCheckerFunction = true' in FastestValidator constructor"); // todo: add link
		}
	...
	compileRule(rule, context, path, innerSrc, resVar) {
		...
		if (item) {
			...
			context.customs[path] = { schema: rule.schema };
			context.index++;
	 ...
	 * @param {Object} opts.context
 	 * @param {Object} opts.messages
	 */
	makeCustomValidator({ vName = "value", fnName = "custom", path, schema, context, messages }) {
		if (typeof schema[fnName] == "function") {
			context.customs[path].messages = messages;
			if (this.opts.useNewCustomCheckerFunction) {
				return `
					const rule = context.customs["${path}"];
					const fnCustomErrors = [];
					value = rule.schema.${fnName}.call(this, ${vName}, fnCustomErrors, rule.schema, "${path}", parent, context);
