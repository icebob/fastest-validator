...
function Validator(opts) {
	...
	// place to store custom messages
	this.customMessages = {};
...
Validator.prototype._processRule = function(rule, name, iterate) {
	...
	// Custom Messages - only supports simple Object rule atm.
	if(rule.messages) {
		if (typeof this.customMessages[name] == "undefined" ) this.customMessages[name] = {};
		Object.keys(rule.messages).forEach( (msgKey) => {
			this.customMessages[name][msgKey] = rule.messages[msgKey];
		});
	}
...
Validator.prototype.resolveMessage = function(err) {
    let msg = this.customMessages[err.field] && this.customMessages[err.field][err.type] ? 
			this.customMessages[err.field][err.type] : this.messages[err.type];
