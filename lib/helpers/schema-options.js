 ...
 *
 * When `value` is `undefined`, returns the empty-string literal `""` rather
 * than the string `"undefined"`. This is a defense-in-depth measure: if a
 * future call site ever forwards `undefined` without guarding, the generated
 * code will still be syntactically valid and semantically harmless (an empty
 * string) instead of silently embedding the literal word "undefined".
 *
 * Note: all current call sites already filter out `undefined` before calling
 * this function, so the `undefined` branch is not reachable today — it exists
 * purely as a safety net against future regressions.
 */
...
function toRegExp(type, field, value, { allowedFlags, flags } = {}) {
	...
	if (typeof value === "string") {
		...
		if (allowedFlags && re.flags) {
			for (const f of re.flags) {
