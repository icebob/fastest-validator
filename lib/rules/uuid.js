...
module.exports = function(schema, path, messages) {
	...
	src.push(`
		if (typeof value !== "string") {
			...
			if (!PATTERN.test(val)) {
				${this.makeError({ type: "uuid", field: path, actual: "origValue", messages })}
			} else {
	...
	if(schema.version) {
		src.push(`
				if (schema.version !== version) {
					${this.makeError({ type: "uuidVersion", field: path, expected: schema.version, actual: "origValue", messages })}
				}
		`);
	}
