...
module.exports = function checkArray(value, schema) {
	...
	if (schema.unique === true && arrLength > Array.from(new Set(value)).length) {
		return this.makeError("arrayUnique", Array.from(new Set(value.filter((item, index) => value.indexOf(item) !== index))));
	}
