...
module.exports = function(schema, path, messages, context) {
	...
	src.push(`
		if (!Array.isArray(value)) {
			${this.makeError({ type: "array", actual: "origValue", messages })}
		} else {
	...
	if (schema.empty === false) {
		src.push(`
			if (len === 0) {
				${this.makeError({ type: "arrayEmpty", actual: "origValue", messages })}
			}
		`);
	}
	...
	if (schema.contains != null) {
		src.push(`
			if (value.indexOf(${JSON.stringify(schema.contains)}) === -1) {
				${this.makeError({ type: "arrayContains", expected: JSON.stringify(schema.contains), actual: "origValue", messages })}
			}
		`);
	}
	...
	if (schema.items != null) {
		...
		src.push(`
			...
			for (let i = 0; i < arr.length; i++) {
				let value = arr[i];
				let origValue = value;
				field = (parentField ? parentField : "") + "[" + i + "]";
		`);
