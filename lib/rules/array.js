...
module.exports = function(schema, path, messages, context) {
	...
	src.push(`
		if (!Array.isArray(value)) {
			${this.makeError({ type: "array", field: path, actual: "value", messages })}
		} else {
	...
	if (schema.empty === false) {
		src.push(`
			if (len === 0) {
				${this.makeError({ type: "arrayEmpty", field: path, actual: "value", messages })}
			}
		`);
	}
	...
	if (schema.contains != null) {
		src.push(`
			if (value.indexOf(${JSON.stringify(schema.contains)}) === -1) {
				${this.makeError({ type: "arrayContains", field: path, expected: JSON.stringify(schema.contains), actual: "value", messages })}
			}
		`);
	}
