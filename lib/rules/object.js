...
module.exports = function(schema, path, messages, context) {
	...
	src.push(`
		if (typeof value !== "object" || value === null || Array.isArray(value)) {
			${this.makeError({ type: "object", field: path, actual: "origValue", messages })}
		} else {
	`);
	...
	if (subSchema) {
		...
		src.push("let parentField = field;");
		...
		for (let i = 0; i < keys.length; i++) {
			...
			const safePropName = `parent${safeSubName}`;
			const newPath = (path ? path + "." : "") + property;
			//const valueName = `value_${cnt}`;

			src.push(`\n// Field: ${newPath}`);
			src.push("if (true) {");
			src.push(`\tlet field = parentField ? parentField + "${safeSubName}" : "${name}";`);
			src.push(`\tlet value = ${safePropName};`);
			src.push("\tlet origValue = value;");
			...
			if (rule.schema.optional === true || rule.schema.type == "forbidden") {
				...
				src.push(`
				if (value === undefined || value === null) {
					${this.makeError({ type: "required", field: newPath, actual: "origValue", messages: rule.messages })}
				} else {`);
			}
			...
			src.push("\tfield = parentField;"); // if (true)
			src.push("}"); // if (true)
		}
