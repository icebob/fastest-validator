...
module.exports = function checkString(schema, path, messages) {
	...
	src.push(`
		if (typeof value !== "string") {
			${this.makeError({ type: "string", actual: "origValue", messages })}
		} else {
	`);
	...
	if (schema.empty === false) {
		src.push(`
			if (len === 0) {
				${this.makeError({ type: "stringEmpty", field: path, actual: "value", messages })}
			}
		`);
	}
	...
	if (schema.pattern != null) {
		...
		src.push(`
			if (!${pattern.toString()}.test(value))
				${this.makeError({ type: "stringPattern", field: path, expected: "\"" + pattern.toString() + "\"", actual: "origValue", messages })}
		`);
	}
