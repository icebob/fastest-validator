...
describe("Test rule: currency", () => {
	it("should have decimal optional, and correctly placed if present", () => {
		...
		expect(check("$1.22.00")).toEqual([
			{
				actual: "$1.22.00",
				field: undefined,
				message: "The '' must be a valid currency format",
				type: "currency",
			},
		]);
	});
	...
	it("should check thousand separator placement is correct", () => {
		...
		expect(check("$1234,222.2")).toEqual([
			{
				actual: "$1234,222.2",
				field: undefined,
				message: "The '' must be a valid currency format",
