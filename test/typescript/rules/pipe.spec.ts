...
describe("TypeScript Definitions", () => {
  describe("Test rule: pipe", () => {
    it("should run nested pipes and merge transformations", () => {
      const schema = {
        payload: { type: "pipe", steps: [
          ...
          {
            type: "pipe", steps: [
            { type: "custom", check(value: string, errors: { type: string; }[]) {
              try {
    ...
    it("should work correctly in async mode", async () => {
      const check = v.compile({ $$async: true, a: { type: "pipe", steps: [{ type: "custom", async check(value: string) {
