...
describe("Test rule: record", () => {
	it("should check values", () => {
		...
		expect(check(false)).toEqual([
			{ type: "record", actual: false, message },
		]);
		// @ts-expect-error
		expect(check(true)).toEqual([
			{ type: "record", actual: true, message },
		]);
		expect(check([])).toEqual([{ type: "record", actual: [], message }]);
		...
		expect(check({ nonNumeric: 3 })).toEqual([
			{
				...
				actual: "nonNumeric",
				field: "nonNumeric",
				message: "The 'nonNumeric' key must be a numeric string.",
			},
		]);
	});
		...
		expect(check({ John: "Doe", Jane: 33 })).toEqual([
			{
				...
				actual: "Doe",
				field: "John",
				message: "The 'John' field must be a number.",
			},
		]);
	});
		...
		expect(check({ John: "Doe", 1: 2 })).toEqual([
			{
				...
				actual: "1",
				field: "1",
				message: "The '1' key must be an alphabetic string.",
			},
			{
				type: "string",
				actual: 2,
				field: "1",
				message: "The '1' field must be a string.",
			},
		]);
	});
	...
	it.each([
		...
		{
			rule: { type: "uuid" },
			value: "10ba038e-48da-487b-96e8-8d3b99b6d18a",
		},
	])(
		"should pass validation when schema has '$rule.type' rule as value",
		({ rule, value }) => {
			const check = v.compile({
				$$root: true,
				type: "record",
