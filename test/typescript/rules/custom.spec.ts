import Validator, { ValidationSchema, CheckerFunction } from "../../../";
...
describe("Test rule: custom v1", () => {
	...
	it("should call custom checker", () => {
		...
		const schema = { $$root: true, type: "custom", a: 5, check: checker } satisfies ValidationSchema;
	...
	it("should call custom checker", () => {
		...
		const schema = {
			weight: { type: "custom", a: 5, check: checker }
	...
	it("should handle returned errors", () => {
		...
		const checker = vi.fn(fn);
		const schema = { weight: { type: "custom", a: 5, check: checker, messages: { myError: "My error message. Expected: {expected}, actual: {actual}, field: {field}" } } } satisfies ValidationSchema;
		const check = v.compile(schema);
